...
with TOP_PRODUCTS as(
        ...
        round(sum(oi.total_sales))  as TOTAL_SALE_VALUE
        from product p
...
SELECT
c.category_name,
round(SUM(oi.total_sales),2) AS total_revenue,
round(
(sum(oi.total_sales)/
(select sum(total_sales) from order_item oi ))
...
round(sum(oi.total_sales),2) total_sale,
dense_rank() over (partition by ca.category_name order by sum(oi.total_sales) ) as least_selling_category 
FROM orders o
